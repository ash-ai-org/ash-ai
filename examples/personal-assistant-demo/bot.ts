/**
 * Personal Assistant — Example application using the Ash SDK.
 *
 * Local usage:
 *   const bot = new AssistantBot('http://localhost:4100');
 *   await bot.setup('assistant', './agent_folder');
 *   const answer = await bot.ask('What is 2+2?');
 *   await bot.teardown();
 *
 * Remote usage (EC2/GCP):
 *   const bot = new AssistantBot('http://<ec2-ip>:4100', 'your-api-key');
 *   await bot.resume('assistant', 'session-id-here');
