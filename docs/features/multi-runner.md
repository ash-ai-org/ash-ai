...
- **Single coordinator is a SPOF.** For horizontal coordinator scaling, see [Multi-Coordinator](./multi-coordinator.md). The runner registry is DB-backed, so adding coordinators requires only Postgres/CRDB and a load balancer.
