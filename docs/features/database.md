...
| Table | Purpose |
|-------|---------|
| `agents` | Deployed agent definitions (tenant-scoped) |
| `sessions` | Session lifecycle state, runner assignment |
| `sandboxes` | Sandbox process tracking for pool management |
| `runners` | Remote runner registry (host, port, capacity, heartbeat) |
| `messages` | User/assistant message history per session |
| `session_events` | Timeline events (tool use, text, errors, lifecycle) |
| `api_keys` | Multi-tenant API key authentication |

The `runners` table enables multi-coordinator deployments — see [Multi-Coordinator](./multi-coordinator.md).
